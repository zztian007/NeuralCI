...
def prepare_data_with_unique(data_dir, func_rep_stg, unique_funs_file, family, return_fun_names=False):
    """
...
def prepare_data(data_dir, func_rep_stg, family, return_fun_names=False):
    """
    Prepare data for compiler family identification task
    Data preparation for other tasks can be easily implemented with slight modifications to this method
    :param data_dir:
...
def explain_with_saliency_map(fig_prefix, model_type, cp_family, att_type, input_files=None, func_list=None, top_k=10):
    """
    ...
    if unique_fun_stg == 'Intact':
        x_test, y_test, func_names = prepare_data(corpus_path, path_stg_suffix, cp_family, True)
    else:
        x_test, y_test, func_names = prepare_data_with_unique(corpus_path, path_stg_suffix, unique_fun_path, cp_family, True)
...
if __name__ == '__main__':
    ...
    model_types = ['bidirectional_cudnn', 'cnn_rnn', 'cudnn_ver']
    attention_types = ['attention_with_context', 'seq_weighted_attention', 'scaled_dot', 'seq_self_attention']
    # model_types = ['naive']
    # attention_types = ['attention_with_context']
            ...
            for model_type in model_types:
                for cp_family in cp_families:
                    ...
                    for att_type in attention_types:
                        # form: 'model type # attention type # the task # instruction abstraction strategy # the unique function extraction strategy'
                        # such as 'cudnn_ver#scaled_dot#cp-family#fine#medium'
                        model_name = model_type + '#' + att_type + '#earlystop-tripleOptimization#' + ins_abs_stgs[
                        ...
                        run_whole_procedure(model_name.replace('.h5', ''), model_type, cp_family, att_type)
