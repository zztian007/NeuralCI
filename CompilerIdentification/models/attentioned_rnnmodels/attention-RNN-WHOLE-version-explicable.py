...
def prepare_data_with_unique(data_dir, func_rep_stg, unique_funs_file, family, ver_list, return_fun_names=False):
    """
...
def prepare_data(data_dir, func_rep_stg, family, ver_list, return_fun_names=False):
    """
    Prepare data for compiler family identification task
    Data preparation for other tasks can be easily implemented with slight modifications to this method
    :param data_dir:
...
def run_whole_procedure(fig_prefix, model_type, cp_family, ver_list, att_type):
    """
...
def explain_with_saliency_map(fig_prefix, model_type, cp_family, ver_list, att_type, input_files=None, func_list=None, top_k=10):
    """
    ...
    if unique_fun_stg == 'Intact':
        x_test, y_test, func_names = prepare_data(corpus_path, path_stg_suffix, cp_family, ver_list, True)
    else:
        x_test, y_test, func_names = prepare_data_with_unique(corpus_path, path_stg_suffix, unique_fun_path, cp_family, ver_list, True)
            ...
            for unique_fun_stg in unique_fun_stgs:
                for model_type in model_types:
                    for att_type in attention_types:
                        ...
                        model_name = model_type + '#' + att_type +'#earlystop-minor-version#' + ins_abs_stgs[
                        ...
                        run_whole_procedure(model_name.replace('.h5', ''), model_type, cp_family, versions[cp_family], att_type)
