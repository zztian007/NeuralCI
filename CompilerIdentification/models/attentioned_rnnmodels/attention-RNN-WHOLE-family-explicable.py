...
def prepare_data_with_unique(data_dir, func_rep_stg, unique_funs_file, return_fun_names=False):
    """
    ...
    # padded_data = sequence.pad_sequences(texts, maxlen=MAX_SEQUENCE_LENGTH, padding='post', truncating='post')
    padded_data = sequence.pad_sequences(texts, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
    if return_fun_names:
...
def prepare_data(data_dir, func_rep_stg, return_fun_names=False):
    """
    Prepare data for compiler family identification task
    Data preparation for other tasks can be easily implemented with slight modifications to this method
    :param data_dir:
...
def run_whole_procedure(fig_prefix, model_type, att_type):
    """
...
def explain_with_saliency_map(fig_prefix, model_type, att_type, input_files=None, func_list=None, top_k=10):
    """
    ...
    if unique_fun_stg == 'Intact':
        x_test, y_test, func_names = prepare_data(corpus_path, path_stg_suffix, True)
    else:
        x_test, y_test, func_names = prepare_data_with_unique(corpus_path, path_stg_suffix, unique_fun_path, True)
        ...
        for unique_fun_stg in unique_fun_stgs:
            for model_type in model_types:
                for att_type in attention_types:
                    ...
                    model_name = model_type + '#' + att_type + '#earlystop-family#' + ins_abs_stgs[
                        i] + '#' + unique_fun_stg + '.h5'
                    print(model_name)
                    ...
                    run_whole_procedure(model_name.replace('.h5', ''), model_type, att_type)