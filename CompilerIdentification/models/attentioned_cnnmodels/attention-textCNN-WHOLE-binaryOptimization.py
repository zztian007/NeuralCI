...
from tensorflow.keras.layers import Embedding, Dense, Conv1D, GlobalMaxPooling1D, Concatenate, Dropout, Reshape
...
from attention_layers import attention_before_convolution, scaled_dot_attention, seq_self_attention
...
class TextCNN(object):
    ...
    def get_model(self, pre_embeddings, dp_rate=-1.0, filter_sizes=[2, 3, 4], attention_dim=100):
        """
        ...
        if self.att_type == 'scaled_dot':
            cxt_vec = scaled_dot_attention.ScaledDotProductAttention(name='Attention')(embedding)
        elif self.att_type == 'seq_self_attention':
            cxt_vec = seq_self_attention.SeqSelfAttention(attention_activation='sigmoid')(embedding)
        elif self.att_type == 'attention_before_convolution':
            cxt_vec = attention_before_convolution.AttentionBeforeConvolution(self.maxlen,
                                                                                attention_hidden_dim=attention_dim)(embedding)
        cxt_vec = Reshape((self.maxlen, self.embedding_dims,))(cxt_vec)
        # concatenate context and input embedding
        embedding = Concatenate()([embedding, cxt_vec])
...
def run_whole_procedure(fig_prefix, cp_family, att_type):
    """
    ...
    text_cnn = TextCNN(MAX_SEQUENCE_LENGTH, len(embeddings_matrix), EMBEDDING_DIM, CLASS_NUMBER, 'softmax', att_type=att_type)
...
if __name__ == '__main__':
    ...
    # ins_abs_stgs = ['medium', 'coarse', 'fine']     # instruction abstraction strategies
    cp_families = ['clang', 'gcc', 'icc']
    attention_types = ['seq_self_attention']
    # attention_types = ['scaled_dot', 'seq_self_attention', 'attention_before_convolution']
    ...
    hyper_log_prefix = 'atten-binaryoptimization'
        ...
        for unique_fun_stg in unique_fun_stgs:
            for cp_family in cp_families:
                ...
                for att_type in attention_types:
                    ...
                    # such as 'cp-optimization#fine#medium#scaled_dot'

                    model_name = 'cp-binaryOptimization#' + ins_abs_stgs[i] + '#' + unique_fun_stg + \
                             '#' + cp_family + '#' + att_type + '.h5'
                    print('-------------------' + model_name + '-------------------')
                    ...
                    run_whole_procedure(model_name.replace('.h5', ''), cp_family, att_type)
