...
class RNNModels(object):
    ...
    def get_naive_version2_model(self, pre_embeddings, dp_rate=0.0, use_lstm=False):
        """
    ...
    def get_multilayer_stateful_model(self, pre_embeddings, dp_rate=0.0, use_lstm=False):
        ...
        if use_lstm:
            model.add(LSTM(RNN_DIM, return_sequences=True, stateful=True))  # returns a sequence of vectors of dimension RNN_DIM
            model.add(LSTM(RNN_DIM, return_sequences=True, stateful=True))  # returns a sequence of vectors of dimension RNN_DIM
            model.add(LSTM(RNN_DIM, recurrent_dropout=dp_rate, stateful=True))  # return a single vector of dimension RNN_DIM
        else:
            model.add(GRU(RNN_DIM, return_sequences=True, stateful=True))  # returns a sequence of vectors of dimension RNN_DIM
            model.add(GRU(RNN_DIM, return_sequences=True, stateful=True))  # returns a sequence of vectors of dimension RNN_DIM
            model.add(GRU(RNN_DIM, recurrent_dropout=dp_rate, stateful=True))  # return a single vector of dimension 128
...
def prepare_data_with_unique(data_dir, func_rep_stg, unique_funs_file, family, ver_list):
    """
                ...
                if line.startswith('>>>'):
                    if line.startswith('>>>Func'):
                        ...
                        if f_label in unique_funs:
                            ...
                            labels.append(label_maps[compiler_version])
                    continue
...
def prepare_data(data_dir, func_rep_stg, family, ver_list):
    """
    Prepare data for compiler family identification task
    Data preparation for other tasks can be easily implemented with slight modifications to this method
    :param data_dir:
...
def run_whole_procedure(fig_prefix, model_type, cp_family, ver_list):
    """
    ...
    if model_type == 'cudnn_ver':
        model = rnn_models.get_cudnn_version_model(embeddings_matrix, DROP_OUT)
    elif model_type == 'naive_ver2':
        model = rnn_models.get_naive_version2_model(embeddings_matrix, DROP_OUT)
    elif model_type == 'cnn_rnn':
        model = rnn_models.get_cnn_rnn_model(embeddings_matrix, DROP_OUT)
    elif model_type == 'bidirectional_cudnn':
        model = rnn_models.get_bidirectional_cudnn_model(embeddings_matrix, DROP_OUT)
    elif model_type == 'naive':
        model = rnn_models.get_naive_version_model(embeddings_matrix, DROP_OUT)
    ...
    if unique_fun_stg == 'Intact':
        texts, labels = prepare_data(corpus_path, path_stg_suffix, cp_family, ver_list)
    else:
        texts, labels = prepare_data_with_unique(corpus_path, path_stg_suffix, unique_fun_path, cp_family, ver_list)
...
if __name__ == '__main__':
    ...
    #
    # if data_location == '1':
    #     corpus_base_path = '/data/zpt5059/compilerprovenance/PhaseI-PathExtraction/'
    #     dic_base_path = '/data/zpt5059/compilerprovenance/PhaseII-ins2vec/'
    #     model_store_base_path = '/data/zpt5059/compilerprovenance/Trained-Models/'
    #     unique_fun_base_path = '/data/zpt5059/compilerprovenance/UniqueFunList-Strict/'
    #
    ...
    #     corpus_base_path = '/dgxhome/zpt5059/projects/compilerprovenance/data/testordered/'
    #     dic_base_path = '/dgxhome/zpt5059/projects/compilerprovenance/data/PhaseII-ins2vec/'
    #     model_store_base_path = '/dgxhome/zpt5059/projects/compilerprovenance/data/Trained-Models/'
    #     unique_fun_base_path = '/dgxhome/zpt5059/projects/compilerprovenance/data/UniqueFunList/'

    corpus_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/PhaseI-OrderedPaths/'
    dic_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/PhaseII-ins2vec/'
    model_store_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/Trained-Models/'
    unique_fun_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/UniqueFunList-Strict/'

    # corpus_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/testordered/'
    # dic_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/PhaseII-ins2vec/'
    # model_store_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/Trained-Models/'
    # unique_fun_base_path = '/home/zztian/workspace/benchmarks/compilerprovenance/UniqueFunList/'
    ...
    # unique_fun_stgs = ['coarse', 'medium', 'fine', 'Intact']
    ...
    for cp_family in versions.keys():
        ...
        label_strs = [cp_family+'-'+ver for ver in versions[cp_family]]
        hyper_log_prefix = 'earlystop-minor-version-'+cp_family
        ...
        for i in range(len(ins_abs_stgs)):
            ...
            for unique_fun_stg in unique_fun_stgs:
                for model_type in model_types:
                    # form: 'model type # the task # the instruction abstraction strategy # the unique function extraction strategy'
                    # such as 'cnn_rnn#cp-family#fine#medium'
                    model_name = model_type + '#earlystop-minor-version#' + ins_abs_stgs[
                        i] + '#' + unique_fun_stg + '#' + cp_family + '.h5'
                    ...
                    run_whole_procedure(model_name.replace('.h5', ''), model_type, cp_family, versions[cp_family])
