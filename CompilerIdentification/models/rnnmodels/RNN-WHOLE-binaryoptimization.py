...
class RNNModels(object):
    ...
    def get_multilayer_stateful_model(self, pre_embeddings, dp_rate=0.0, use_lstm=False):
        ...
        if use_lstm:
            ...
            model.add(GRU(RNN_DIM, return_sequences=True, stateful=True))  # returns a sequence of vectors of dimension RNN_DIM
            model.add(GRU(RNN_DIM, recurrent_dropout=dp_rate, stateful=True))  # return a single vector of dimension RNN_DIM
    ...
    def get_multilayer_model(self, pre_embeddings, dp_rate=0.0, use_lstm=False):
        ...
        if use_lstm:
            ...
            model.add(GRU(RNN_DIM, return_sequences=True))  # returns a sequence of vectors of dimension RNN_DIM
            model.add(GRU(RNN_DIM, recurrent_dropout=dp_rate))  # return a single vector of dimension RNN_DIM
...
def prepare_data_with_unique(data_dir, func_rep_stg, unique_funs_file, family):
    """
    ...
    :param family: just consider data files correlated with this specified family
    :return:
    """
                ...
                if line.startswith('>>>'):
                    if line.startswith('>>>Func'):
                        ...
                        if f_label in unique_funs:
                            ...
                            labels.append(label_maps[compiler_opt_level])
                    continue
...
def prepare_data(data_dir, func_rep_stg, family):
    """
    Prepare data for compiler family identification task
    Data preparation for other tasks can be easily implemented with slight modifications to this method
    :param data_dir:
            ...
            for line in lines:
                ...
                for word in line.split(' '):
                    try:
                        ...
                        indices.append(word_index['UNKNOWN'])  # OOV词统一用'UNKNOWN'对应的向量表示                texts.append(indices)
                texts.append(indices)
                labels.append(label_maps[compiler_opt_level])
            f.close()
    print('transforming labels to array')
...
def run_whole_procedure(fig_prefix, model_type, cp_family):
    """
    ...
    if unique_fun_stg == 'Intact':
        ...
        texts, labels = prepare_data_with_unique(corpus_path, path_stg_suffix, unique_fun_path, cp_family)
...
if __name__ == '__main__':
    ...
    hyper_log_prefix = 'earlystop-binaryoptimization'
    log_config(hyper_log_prefix)

    model_types = ['cudnn_ver', 'cnn_rnn', 'bidirectional_cudnn']
    ...
    for i in range(len(ins_abs_stgs)):
        ...
        for unique_fun_stg in unique_fun_stgs:
            for model_type in model_types:
                for cp_family in cp_families:
                    label_strs = [cp_family + '-' + ver for ver in ['Low', 'High']]
                    ...
                    model_name = model_type + '#earlystop-binaryOptimization#' + ins_abs_stgs[
                    ...
                    run_whole_procedure(model_name.replace('.h5', ''), model_type, cp_family)
