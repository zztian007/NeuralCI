...
def run_whole_procedure(fig_prefix, model_type):
    """
    ...
    print('training...')
    start = time.clock()
    model = neural_net_train(model, x_train=x_train, y_train=y_train, validation_data=(x_test, y_test), fig_prefix=fig_prefix)
    end = time.clock()
...
if __name__ == '__main__':
    ...
    # modify the hyper-parameters here
    lr = 1e-3  # learning rate
    ...
    hyper_log_prefix = 'earlystop-familyandtripleoptimization'
    ...
    for i in range(len(ins_abs_stgs)):
        ...
        for unique_fun_stg in unique_fun_stgs:
            for model_type in model_types:
                ...
                model_name = model_type + '#earlystop-FamilyandTripleOptimization#' + ins_abs_stgs[
                    i] + '#' + unique_fun_stg + '.h5'
                ...
                run_whole_procedure(model_name.replace('.h5', ''), model_type)
