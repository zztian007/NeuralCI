                ...
                if line.startswith('>>>'):
                    if line.startswith('>>>Func'):
                        ...
                        if f_label in unique_funs:
                            ...
                            labels.append(label_maps[compiler_family+'-'+compiler_version])
                    continue
...
if __name__ == '__main__':
    ...
    for cp_family in versions.keys():
        ...
        for ver in versions[cp_family]:
            label_maps[cp_family + '-' + ver] = i
            i += 1
            label_strs.append(cp_family + '-' + ver)
    CLASS_NUMBER = len(label_maps)

    hyper_log_prefix = 'familyAndVersion'
    ...
    for i in range(len(ins_abs_stgs)):
        ...
        for unique_fun_stg in unique_fun_stgs:
            for model_type in model_types:
                ...
                # such as 'cp-optimization#fine#medium#clang'
                model_name = model_type + '#earlystop-familyandversion#' + ins_abs_stgs[
                    i] + '#' + unique_fun_stg + '.h5'
                print('-------------------' + model_name + '-------------------')
