...
def prepare_data_with_unique(data_dir, func_rep_stg, unique_funs_file, versions):
    """
...
def prepare_data(data_dir, func_rep_stg, versions):
    """
    Prepare data for compiler family identification task
    Data preparation for other tasks can be easily implemented with slight modifications to this method
    :param data_dir:
...
def run_whole_procedure(fig_prefix, model_type, versions):
    """
    ...
    for cp_family in versions.keys():
        ...
        for ver in versions[cp_family]:
            for opt in cp_optimization_options:
                label_maps[cp_family+'-'+ver+'-'+opt] = i // 2
                i += 1
                if opt == 'O1':
                    label_strs.append(cp_family + '-' + ver + '-Low')
                    continue
                elif opt == 'O3':
                    label_strs.append(cp_family + '-' + ver + '-High')
    CLASS_NUMBER = len(label_strs)
    ...
    for i in range(len(ins_abs_stgs)):
        ...
        for unique_fun_stg in unique_fun_stgs:
            for model_type in model_types:
                ...
                model_name = model_type + '#earlystop-allsettings-binary#' + ins_abs_stgs[
                    i] + '#' + unique_fun_stg + '.h5'
                ...
                run_whole_procedure(model_name.replace('.h5', ''), model_type, versions)
